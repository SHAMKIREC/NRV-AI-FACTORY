...
- Pre-auth role visibility/action contract is now frozen in `docs/ROLE_MATRIX.md`: Developer owner, Admin, Dispatcher, Accountant and Viewer; server-side enforcement and 401/403 negative tests remain gates before mandatory auth.
- Current verified LOGIX implementation head before the documentation-only role-matrix commit: `e9006cffbb0160c7c6ada3df192df632f2162cf5` (`test(trips): lock idempotent create contract`). CI run `35133949863` / #291 is green for that exact head.
- Exact-head Vercel production deployment `dpl_9bUKL8jiJR31CpnieNyx97j9Eooy` was READY and `/api/health` returned HTTP 200 with `database: ok`, `authMode: demo` and matching commit SHA.
...
1. Verify the documentation-only role-matrix head through CI/Vercel; then add server-side role enforcement incrementally with negative tests before any auth switch.
